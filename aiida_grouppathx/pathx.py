...
from aiida.tools.groups.paths import GroupPath, NoGroupsInPathError, InvalidPath, GroupAttr, REGEX_ATTR
...
class GroupPathX(GroupPath):
    ...
    def __init__(self,
                 ...
                 warn_invalid_child: bool = True) -> None:
        ...
        super().__init__(path=path,
                         cls=cls,
                         warn_invalid_child=warn_invalid_child)
    ...
    def _get_node_query(self) -> Optional[orm.QueryBuilder]:
        ...
        query.append(parent.cls,
                     subclassing=False,
                     filters={'label': parent.path})
    ...
    def children(self) -> Iterator['GroupPath']:
        ...
        node_query.append(self.cls,
                          subclassing=False,
                          filters={'label': self.path})
        node_query.append(orm.Node,
                          with_group=self.cls,
                          filters={'extras': {
                              'has_key': self._extras_key
                          }},
            ...
            if (path_string not in yielded
                ...
                try:
                    ...
                    if self._warn_invalid_child:
                        warnings.warn(
                            f'invalid path encountered: {path_string}')  # pylint: disable=no-member
    ...
    def add_node(self, node: Union[orm.Node, 'GroupAttr'], alias:str, force=False):
        ...
        if not REGEX_ATTR.match(alias):
            warnings.warn(f'Alias {alias} is not valid Python identifier - you may consider using one for easier access.')
        if self.is_virtual:
